...
@onready var animation_player = $TrueLawlorCharacter/AnimationPlayer
...
@onready var skeleton : Skeleton3D = $TrueLawlorCharacter/MBlab_sk1733170350_0849836/Skeleton3D
@onready var healthBar : ProgressBar = $"../LawlorHealthBar"
var blocking = false
...
func update_animation_parameters(delta):
	...
	# Walk Right
	if Input.is_action_pressed("LawlorRight") and !blocking and !attacking:
		...
		if (!facingRight):
			$TrueLawlorCharacter.rotation.y += PI
			facingRight = true
	
	# Walk Left
	elif Input.is_action_pressed("LawlorLeft") and !blocking and !attacking:
		animationTree["parameters/conditions/isWalking"] = true
		animationTree["parameters/conditions/idle"] = false
		head_vuln.monitoring = true
		head.disabled = false
	...
	elif Input.is_action_just_pressed("LawlorLeftHigh") and !blocking:
	...
	elif Input.is_action_just_pressed("LawlorLeftLow") and !blocking:
	...
	elif Input.is_action_just_pressed("LawlorRightHigh") and !blocking:
	...
	elif Input.is_action_just_pressed("LawlorRightLow") and !blocking:
	...
	# Blocking
	elif Input.is_action_pressed("LawlorGuardHigh") and !blockingLow:
	...
	elif Input.is_action_pressed("LawlorGuardLow") and !blockingHigh:
		...
		blockingLow = true
	elif Input.is_action_just_pressed("LawlorSkinwalker"):
